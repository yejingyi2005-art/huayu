...
- [x] 个人资料展示
- [x] 头像上传（压缩 → IndexedDB 存储）
- [x] 昵称编辑（持久化到 localStorage）
- [x] 统计看板（照片/文字/心情数量、心情分布条形图、时光统计）
- [x] 花园列表（可跳转）
- [x] 深色模式切换
- [x] 数据导出（JSON 下载）
- [x] 数据清除
...
- [x] 单元测试：服务层函数（33 tests, 7 files）
- [x] 集成测试：核心业务流（7 tests — 创建花园、留痕、时间线分组、纪念册 getOrCreate、心情映射、跨服务全流程）
- [x] E2E 测试：创建空间 → 留痕 → 回顾（2 tests — core flow + auth protection, Playwright）
...
| 动效实现 | ✅（全部 6 项完成） |
| AI 能力（V2） | ⬜（V2 阶段） |
| 测试 | ✅（35 tests — 33 unit/integration + 2 E2E Playwright） |
| 验收 | ✅（全部通过） |
